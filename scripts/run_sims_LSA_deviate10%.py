...
import yaml
import re   
### does it make sense to vary the parameters by 10%?
### some of the parameters are already on the boundary

# %%

with open(config.potato_paramters, 'rb') as f:
    # The protocol version used is detected automatically, so we do not
    # have to specify it.
    params_potato = yaml.load(f, Loader=yaml.FullLoader)
...
# Flatten the params_potato dictionary
flat_params_potato = flatten_dict(params_potato)
matching_keys = []
cultivar_info = []
matching_values = []

for i in flat_params_potato.items():
    for var in config.problem['names']:
        if re.search(config.variety_name + '_' + var, i[0]):
            print(i[0])
            matching_keys.append(var)
            cultivar_info.append(config.variety_name)
            matching_values.append(i[1][0])

# Create a DataFrame from the matching data
matches_df = pd.DataFrame({ 'Cultivar' : cultivar_info, 'Key': matching_keys,'Value': matching_values})
