...
def plot_sorted_df(day, output_var='TWSO', method='Saltelli', samplesize=32768, sortby='S1'):
    ...
    if isinstance(day, list):
        ...
        for i, (d, ax) in enumerate(zip(day, axs.flatten())):
            # Load data
            Si = load_data(d, output_var, method)
            df = to_df(Si[f'si_day_{d}_{output_var}'])
            ...
            barplot = Sis.plot(kind='barh', yerr=confs, ax=ax)
        ...
        plt.xlim(0, 1)
        fig.text(0.5, 0.05, 'Sensitivity index', ha='center')
